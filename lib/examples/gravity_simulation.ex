defmodule Examples.GravitySimulation do
  ...
  def solar_system do
    ...
    earth = %{
      name: "Earth",
      mass: 5.972e24,
      position: {1.496e11, 0.0, 0.0},  # AU in meters
      velocity: {0.0, 2.978e4, 0.0},   # m/s
      radius: 6.371e6
    }
    ...
    moon = %{
      name: "Moon",
      mass: 7.342e22,
      position: {1.496e11 + 3.844e8, 0.0, 0.0},  # Earth-Moon distance
      velocity: {0.0, 2.978e4 + 1.022e3, 0.0},   # m/s
      radius: 1.737e6
    }
  ...
  def step_simulation(system, dt) do
    updated_bodies = Enum.map(system.bodies, fn body ->
      # Calculate total force on this body
      total_force = Enum.reduce(system.bodies, {0.0, 0.0, 0.0}, fn other_body, acc ->
        if body != other_body do
          force = gravitational_force(body, other_body, system.g)
          {acc.x + force.x, acc.y + force.y, acc.z + force.z}
        else
          acc
        end
      end)
  ...
  def total_energy(system) do
    kinetic = Enum.sum(Enum.map(system.bodies, fn b ->
      0.5 * b.mass * (b.velocity.x*b.velocity.x + b.velocity.y*b.velocity.y + b.velocity.z*b.velocity.z)
    end))

    potential = Enum.sum(for b1 <- system.bodies, b2 <- system.bodies, b1 != b2 do
    ...
    end) / 2  # Divide by 2 because each pair is counted twice
